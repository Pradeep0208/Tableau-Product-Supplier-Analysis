#  Product & Supplier Financial Insights – Tableau Dashboard
...
1. **Avg Quantity vs Gross Revenue (Dual Axis)**  
   → Compares product popularity vs profitability in a single chart.

2. **Daily Avg Gross Revenue by Order Date**  
   → Highlights trends and seasonality across orders over time.

3. **Daily Avg Revenue (By Order Date & Category)**  
   → Drill-down visualization to monitor revenue by product category.

4. **Map – Financial Measure by Country**  
   → Geo-visualization to monitor performance across regions.

5. **Bottom N Products by Gross Revenue**  
   → Helps identify least profitable products based on a dynamic N filter.

6. **Suppliers with Products Above Net Revenue Threshold**  
   → Highlights high-performing suppliers based on user-defined thresholds.

7. **Product Revenue vs Cost Analysis**  
   → Scatter plot for margin analysis between revenue and cost.

8. **Top N Products (by Gross Revenue)**  
   → Easily view best-sellers through a parameterized N selector.

9. **Financial Summary by Year**  
   → Year-over-year comparison of all key financial KPIs.

10. **Category-wise Gross Revenue vs Cost**  
    → Boxplots for financial spread within categories.

11. **Dynamic Financial Measure Selector**  
    → Switch between metrics (Gross, Net, Cost) across all visuals.

12. **Supplier-Level Filtering and Analysis**  
    → See which suppliers contribute to high/low performance with filters.
